<template>
  <div class="cart">
    ...
    <ul>
      <li v-for="(book,idx) in cartBookList" :key="book.id">
        ...
        <div class="cart-select-box">
          <i class="cart-not-select" v-if="book.notSelect" @click="showSelect(idx)">X</i>
          <i class="cart-select" v-if="book.select" @click="showNotSelect(idx)">√</i>
        </div>
    ...
    <div class="cart-pay">
      <button @click="showcenter">去结算</button>
    </div>
    <transition name="run">
      <paycenter v-show="show" @close="closecenter" :money="money"/>
    </transition>
  </div>
</template>
